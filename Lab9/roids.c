...
//printf("ship->x: %d ship->x+dx: %d \n", ship->x, ship->x+dx);
  ...
  void draw_triangle( float x1, float y1, 
        ...
        float x3, float y3,
        const float color[4] )
  draws a triangle with specified corners in the specified color
  float dx; // movement in x per update step
  float dy; // movement in y per update step
 s float x; // ship centre position
  float y; // ship centre position
...
// (0,0)            (1,0)
  ...
  //x-dx, y-dy -> opposit end of the ship!
  //thrust must be 1-1
/*    
const unsigned int WIDTH = 700;
const unsigned int HEIGHT = 700;
...
//int py = 0;
  ...
  //  float x1 = ship->x+dx < 0 ? ship->x+dx+1 : (ship->x+dx > 1 ? ship->x+dx-1 : ship->x+dx);
  //  float y1 = ship->y+dy < 0 ? ship->y+dy+1 : (ship->y+dy > 1 ? ship->y+dy-1 : ship->y+dy);

   // x1 = ship->x+dx > 1 ? ship->x+dx-1 : ship->x+dx;
   // y1 = ship->y+dy > 1 ? ship->y+dy-1 : ship->y+dy;

  //  float x2 = ship->x+dx < 0 ? ship->x+lx+1 : (ship->x+lx > 1 ? ship->x+lx-1 : ship->x+lx);
  //  float y2 = ship->x+dx < 0 ? ship->x+rx+1 : (ship->y+ly > 1 ? ship->y+ly-1 : ship->y+ly);

//    x2 = ship->x+dx > 1 ? ship->x+lx-1 : ship->x+lx;
//    y2 = ship->y+dy > 1 ? ship->y+ly-1 : ship->y+ly;

  //  float x3 = ship->x+dx < 0 ? ship->x+rx+1 : (ship->x+rx > 1 ? ship->x+rx-1 : ship->x+rx);
  //  float y3 = ship->y+dy < 0 ? ship->y+ry+1 : (ship->y+ry > 1 ? ship->y+ry-1 : ship->y+ry);

//    x3 = ship->x+dx > 1 ? ship->x+rx-1 : ship->x+rx;
//    y3 = ship->y+dy > 1 ? ship->y+ry-1 : ship->y+ry;
/*
    draw_triangle(x1, y1, 
      x2, y2, 
      x3, y3,
...
if (ship->thrust) {
    if ((ship->x)-dx < 0) {
      draw_triangle(ship->x-dx+1.0, ship->y-dy,
        ship->x+lx+1.0, ship->y+ly,
        ship->x+rx+1.0, ship->y+ry,
        thrustcolor);
    }
    if ((ship->x)-dx > 0) {
      draw_triangle(ship->x-dx-1.0, ship->y-dy,
    ...
    if ((ship->y)-dy > 0) {
      draw_triangle(ship->x-dx, ship->y-dy-1.0,
        ship->x+lx, ship->y+ly-1.0,
        ship->x+rx, ship->y+ry-1.0,
        thrustcolor);
    }
}  

  bool isXneg = ship->x+dx < 0 || ship->x+lx < 0 || ship->x+rx < 0;
  bool isXpos = (ship->x)+dx > 1 || ship->x+rx > 1 || ship->x+lx > 1;
  bool isYneg = (ship->y)+dy < 0 || ship->y+ly < 0 || ship->y+ry < 0;
  bool isYpos = (ship->y)+dy > 1 || ship->y+ly > 1 || ship->y+ry > 1;
  ...
  if (ship->x+dx > 1 && ship->y+dy > 1) {
    draw_triangle(ship->x+dx-1, ship->y+dx-1,
      ship->x+lx-1, ship->y+ly-1,
      ship->x+rx-1, ship->y+ry-1,
      shipcolor);
  }
  if (ship->x+dx < 0 && ship->y+dy > 1) {
    draw_triangle(ship->x+dx+1, ship->y+dy-1,
      ship->x+lx+1, ship->y+ly-1,
      ship->x+rx+1, ship->y+ry-1,
      shipcolor);
  }
  if (ship->x+dx > 1 && ship->y+dy < 0) {
    draw_triangle(ship->x+dx-1, ship->y+dy+1,
      ship->x+lx-1, ship->y+ly+1,
      ship->x+rx-1, ship->y+ry+1,
      shipcolor);
  }
  //off to the left->back to right of the world
  ...
  // off to the right-> back to the left of the world
  if ((ship->x)+dx > 1 || ship->x+rx > 1 || ship->x+lx > 1) {
    draw_triangle(ship->x+dx-1.0, ship->y+dy,
      ship->x+lx-1.0, ship->y+ly,
      ship->x+rx-1.0, ship->y+ry,
      shipcolor);
  }
  // off to the bottom -> back to the top
  ...
  //off to the top -> back to the bot
...
/*
  else if (ship->x+dx < 0 && ship->y+dx > 1) {
    draw_triangle(ship->x+dx+1, ship->y+dx)
  }
  ...
  if (ship->y+dx > 0) {
    draw_triangle(    ship->x+lx, ship->y+dy-1,
    ship->x+lx, ship->y+ly-1,
    ship->x+rx, ship->y+ry-1,
    shipcolor);
  }
...
bool isXneg = (roid->x-roid->width/2.0) < 0;
bool isYneg = (roid->y-roid->height/2.0) < 0;
bool isXpos = (roid->x+roid->width/2.0) > 1;
bool isYpos = (roid->y+roid->height/2.0) > 1;
float x1 = roid->x-roid->width/2.0;
float x2 = roid->x+roid->width/2.0;
float y1 = roid->y-roid->height/2.0;
float y2 = roid->y+roid->height/2.0;
  if (isXneg) {
    draw_rectangle(x1+1, y1,
      1, y2,
      roid->color);
  }
  if (isYneg) {
    draw_rectangle(x1, y1+1,
      x2, 1,
      roid->color);
  }
  if (isXpos) {
  ...
  if (isXpos && isYneg) {
    draw_rectangle(0, y1+1,
      x2-1, 1,
      roid->color);
  }
  if (isXneg && isYpos) {
    draw_rectangle(x1+1, 0,
      1, y2-1,
      roid->color);
  }
  ...
  //rectangle corner cases (0,1) (0,0) (1,0) (1,1)
  //(0,1)top left ->bot right
  /*
...
/*
if ((shot->x + shot->dx) < 0 && (shot->y + shot->dy) < 0) {
  return (shot->x >= x1+1 &&
    shot->x <= 1 &&
    shot->y >= y1+1 &&
    shot->y <= 1);
...
if ((shot->x + shot->dx) > 1 && (shot->y + shot->dy) > 1) {
  return (shot->x >= 0 &&
  shot->x <=  x2-1 &&
  shot->y >= 0 &&
  shot->y <= y2-1); 
}
if ((shot->x + shot->dx) >1 && (shot->y + shot->dy)< 0) {
  return (shot->x >= 0 &&
    shot->x <= x2-1 &&
    shot->y >= y1+1 &&
    shot->y <= 1);
...
if ((shot->x + shot->dx) < 0 && (shot->y + shot->dy) > 1) {
  return (shot->x >= x1+1 &&
    shot->x <= 1 &&
    shot->y >= 0 &&
    shot->y <= y2-1);
